...
function ListAccessories() {
    ...
    const { isLoading, data, isError, error } = useAccessoriesData();
        ...
        <>
            <div className="card shadow mb-4">
                <div className="card-header py-3">
                    <h6 className="m-0 font-weight-bold text-primary">Quản lý phụ kiện</h6>
                </div>
                <div className="card-body">
                    <div className="table-responsive">
                        <Link to="/admin-add-accessory" className="btn btn-success mb-3">
                        ...
                        <table className="table table-bordered" id="dataTable" width="100%" cellSpacing={0}>
                            ...
                            <tbody>
                                {listProducts && listProducts.length > 0 ? (
                                    listProducts
                                        ?.filter(
                                            (productCategory) => productCategory.productCategoryCode !== 'DIENTHOAI',
                                        )
                                        .map(
                                            (item, index) =>
                                                index >= minIndex &&
