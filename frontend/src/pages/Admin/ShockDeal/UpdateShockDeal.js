...
import { useAddShockDealData, useGetShockDealData, useUpdateShockDealData } from '~/hooks/react-query/shockdealData';
registerLocale('vi', viLocale);
function UpdateShockDeal() {
    ...
    const { id } = useParams();
    const { isLoading, data, isError, error } = useGetShockDealData(id);
    console.log({ data });
    useEffect(() => {
        if (data && data.data) {
            setValue('name', data.data.name);
            const shockDealDetails = data.data.shockDealDetails;
            setStartDate(new Date(data.data.startDate));
            setEndDate(new Date(data.data.endDate));
            const uniqueMainProductIds = [...new Set(shockDealDetails.map((detail) => detail.mainProductId))];
            // Lọc và chuyển đổi thành mảng mainProduct
            const uniqueMainProducts = uniqueMainProductIds.map((mainProductId) => {
                return shockDealDetails.find((detail) => detail.mainProductId === mainProductId).mainProduct;
            });
            const uniqueShockDealIds = [...new Set(shockDealDetails.map((detail) => detail.shockDealProductId))];
            // Lọc và chuyển đổi thành mảng mainProduct
            const uniqueShockDeals = uniqueShockDealIds.map((shockDealProductId) => {
                const detailWithPrice = data.data.shockDealDetails.find(
                    (deal) => deal.shockDealProductId === shockDealProductId,
                );
                return {
                    ...shockDealDetails.find((detail) => detail.shockDealProductId === shockDealProductId)
                        .productShockDeal,
                    shockDealPrice: detailWithPrice ? detailWithPrice.shockDealPrice : null,
                    formatPrice: detailWithPrice ? detailWithPrice.shockDealPrice.toLocaleString() : null,
                };
            });

            setListMainProducts(uniqueMainProducts);
            setListShockDeals(uniqueShockDeals);
        }
    }, [data]);
    ...
    const { mutate: updateShockDeal } = useUpdateShockDealData(onSuccess);
            ...
            <>
                <div className="card shadow mb-4">
                    <div className="card-header py-3">
                        <h6 className="m-0 font-weight-bold text-primary">Cập nhật Deal sốc</h6>
...
export default UpdateShockDeal;
