...
function UpdateProduct() {
    ...
    useEffect(() => {
        if (data) {
            ...
            const convertedOptions = data.data.productSamples.map((option) => ({
            ...
            setValue('technicalDetail', data.data.technicalDetail);
            setValue('infomation', data.data.infomation);
    ...
    if (isLoadingBrands || isLoadingColorProducts || isLoadingWareHouses || isLoading) {
        return <LoadingAdmin />;
    }
    console.log('111111', selectedOption);
    return (
        <>
            <>
