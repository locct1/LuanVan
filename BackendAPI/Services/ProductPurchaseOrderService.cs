...
namespace BackendAPI.Services
{
...

    {
        ...
        {
            return await _unitOfWork.GetRepository<ProductPurchaseOrder>().GetAll(include: p => p.Include(x => x.Supplier).Include(a => a.WareHouse).Include(a => a.User).Include(a => a.ProductPurchaseOrderDetails), orderBy: x => x.OrderByDescending(x => x.Id));
        }
